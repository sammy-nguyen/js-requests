...
/*
    ...
    Bel ow you will find an event listener on a button.
...
const repeatMyParam = () => {
  return axios.get("http://localhost:3000/repeat/hellocoding").then((res) => {
    console.log(res.data);
...
repeatBtn.onclick = repeatMyParam;

//PROBLEM 7
// repeatBtn = document.querySelector('#repeat-text');
...
function getQuery() {
  return axios
    .get("http://localhost:3000/query-test?something=random")
    .then((res) => {
      console.log(res.data);
    });
}
...
// the response is: You sent an empty query!
    2: Send more than 1 query on the URL -- what happened?
    /**
     * the response is: an object with message property saying You sent more than 1 query! and queries properties with 2 key value pairs that I sent in
     */
